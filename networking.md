...
1) **KUBE-SERVICES(CHAIN)**
   Containts all Services 
   ```
...
2) **KUBE-SVC-* the "LoadBalance"**
   Every services has an KUBE-SVC entry
   ```
   ...
   ```
3) **KUBE-SEP-* the Service EndPoint**
   Every endpoint of an service. (`oc get endpoints`)
   ```
...
![package flow](images/packet_flow10.png)

Source: http://xkr47.outerspace.dyndns.org/netfilter/packet_flow/